...
const FactControl = (props) => {
  const { cowOrHeifer, breedType, semenType, systemType, gnrh, pg } = props;
  ...
  return (
    <div className="adding-rules-divs">
      ...
      <form>
        <Select
          onChange={(e) => {
            onParamSelectionClick(e.target.value);
          }}
          sx={{ m: 1, minWidth: 190 }}
        >
        ...
        <Select sx={{ m: 1, minWidth: 120 }}>
          ...
          {Object.keys(selectedParamValues).map((k) => {
            if (k !== 0) {
              return <MenuItem key={k}>{selectedParamValues[k].name}</MenuItem>;
            } else {
              return <MenuItem> hello world </MenuItem>;
            }
          })}
        </Select>
      </form>
